...
class Policy_Network(nn.Module):
    def __init__(self, obs_space_dims: int, action_space_dims: int):
        ...
        self.shared_net = nn.Sequential(
            nn.Conv2d(1, nr_kernels_1, 1, dtype=torch.float),
            nn.Sigmoid(),
            ...
            # nn.Sigmoid(),
            nn.Flatten(0, -1),
            nn.Linear(nr_kernels_1*10*10, hidden_space, dtype=torch.float),
            nn.Tanh()
        )
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        o2 = self.l2(o1)
        print(f"{o2 = }")
        shared_features = self.l3(o2)
        print(f"{shared_features = }")
...
class REINFORCE:
    ...
    def __init__(self, obs_space_dims: int, action_space_dims: int):
        ...
        # Hyperparameters
        self.learning_rate = 2e-2  # Learning rate for policy optimization
...
total_num_episodes = int(100)  # Total number of episodes og: 5e3
nr_epochs = int(50)
